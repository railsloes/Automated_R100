...
+*   **Method:** Unit Test (`tests/test_{TEST_NUMBER}.py::test_function_name`)
+*   **Evidence:** 
+    ```
+    Ran 1 test in 0.05s
+    OK
...
**Note:** Assumes Current Working Directory (CWD) is the specific project root (e.g., `/Users/federicolopez/Automated_R100/{PROJECT_NAME}/`). Context includes `SPRINT_NUMBER`, `TASK_NUMBER`, `HYPOTHESIS_NUMBER`.

*   `Sprint_{SPRINT_NUMBER}/Tasks/task_{TASK_NUMBER}_hypothesis_{HYPOTHESIS_NUMBER}.md`: The specific hypothesis to analyze.
*   `Sprint_{SPRINT_NUMBER}/Tasks/task_{TASK_NUMBER}_context.md`: Broader task context.
*   Existing codebase (especially files mentioned in the hypothesis).

**Output:**
*   Updated `Sprint_{SPRINT_NUMBER}/Tasks/task_{TASK_NUMBER}_hypothesis_{HYPOTHESIS_NUMBER}.md`: Section 10 filled with analysis results, logs, and recommendation.
*   Updated `Sprint_{SPRINT_NUMBER}/Tasks/task_{TASK_NUMBER}_hypothesis.json`: Status updated for the analyzed hypothesis (e.g., `code_supported`, `code_contradicted`).
*   Potentially temporary code changes or new test files (which might be discarded or committed depending on the outcome).

    *   If yes: Apply the changes described in the hypothesis to the code (locally, potentially on a temporary branch or just in the working directory).
    *   If no: Proceed to Step 3 (Execution/Observation).
2.  **Write Test (If Applicable):** Based on the 'Verification Strategy (Code)' section in the hypothesis file:
    *   If a specific test needs to be written (e.g., a new unit test for `test_{TEST_NUMBER}`), create or modify the necessary test file.
3.  **Execute & Observe:**
    *   Run the relevant test(s) identified in the verification strategy.
    *   If the strategy involves observing application behavior (e.g., API response, log output), execute the necessary steps to trigger that behavior.
    *   Capture relevant output: test results (pass/fail), logs, error messages, observed behavior.
4.  **Analyze Results:** Compare the observed results against the 'Expected Outcome' and 'Potential Side Effects' described in the hypothesis file.
    *   Did the code behave as expected?
    *   Did the tests pass?
    *   Were there any unexpected side effects?
5.  **Determine Outcome:**
    *   **Code Supported:** The execution results align with the expected outcome, and tests pass.
    *   **Code Contradicted:** The results contradict the expected outcome, tests fail, or significant negative side effects occur.
    *   **Inconclusive:** The results are unclear, or the test/observation method was insufficient.
6.  **Update Hypothesis File (`task_{TASK_NUMBER}_hypothesis_{HYPOTHESIS_NUMBER}.md`):**
    *   Go to Section 10 ("Code Analysis Result").
    *   Set the `Status` (e.g., "Code Supported", "Code Contradicted").
    *   Describe the `Method` used (e.g., "Unit Test: `test_{TEST_NUMBER}.py::test_function_name`", "Manual API Call").
    *   Include `Evidence` (paste relevant logs, test output, or a summary of observations).
    *   Summarize the `Outcome` based on Step 5.
    *   Provide a `Recommendation`:
        *   If supported: "Proceed with implementation based on validated hypothesis."
        *   If contradicted: "Discard hypothesis. Select next hypothesis or generate new ones."
        *   If inconclusive: "Refine test/observation method and re-analyze, or discard hypothesis."
7.  **Update Hypothesis List (`task_{TASK_NUMBER}_hypothesis.json`):**
    *   Find the object corresponding to `HYPOTHESIS_NUMBER`.
    *   Update the `status` field based on the outcome (e.g., `code_supported`, `code_contradicted`).
8.  **Clean Up (Optional):** If temporary code changes were made, revert them unless the hypothesis was supported and the next step is immediate implementation.
